...
> **Data:** 2026-08-23
> **Versão do documento:** 1.5 — incorpora o que a primeira instalação real ensinou (delegação D-Bus do atalho, markup do Adwaita, dependências de `venv`, empacotamento sem debhelper); anterior: 1.4 — a bandeja passa a falar StatusNotifierItem direto por D-Bus (a `libayatana-appindicator3` é GTK 3 e não convive com GTK 4); anterior: 1.3 — **a tradução offline passou a ser o provider padrão** (revisão de PA-03 por custo da API, 2026-08-23); marcos reordenados; viabilidade do motor local comprovada empiricamente
...
**IC8 — `Gio.Settings` aborta o processo quando o schema não existe.** Construir um `Gio.Settings` para um schema ausente chama `g_error`, que encerra o processo imediatamente — **não lança exceção**, portanto `try/except` não protege. Como o atalho global depende de `org.gnome.settings-daemon.plugins.media-keys` (IC4), qualquer desktop que não seja GNOME derrubaria o aplicativo. A existência do schema DEVE ser verificada por `Gio.SettingsSchemaSource`, que devolve `None` em vez de abortar, antes de construir qualquer `Settings`. Descoberto porque a CI, que não tem GNOME, morria com `SIGTRAP` mudo.
...
- **RF-09** — O sistema DEVE expor a CLI `translate-linux --capture`, que aciona a captura na instância já em execução **ativando a ação `capture` em `org.freedesktop.Application`** (interface que o `GApplication` já publica), sem iniciar um segundo processo. A ativação DEVE usar `NO_AUTO_START`: a ausência de instância é a resposta desejada, não motivo para criar uma. As formas que pedem saída no terminal (`--json`, `--ocr-only`) DEVEM executar localmente, pois é onde a saída precisa aparecer.
- **RF-09.1** — Um atalho global só sabe executar um comando, então a delegação é o que faz o atalho alcançar a bandeja. Sem ela, o comando inicia um processo paralelo que captura e escreve num terminal inexistente — e o atalho parece não fazer nada. Além disso, a delegação torna irrelevante **qual** `translate-linux` está primeiro no `PATH`: quem detém o nome no barramento executa o trabalho.
...
- **RF-41** — Os modelos NÃO DEVEM ser embutidos no `.deb`. O sistema DEVE oferecer instalação sob demanda, com progresso e opção de remoção, armazenando em `~/.local/share/translate-linux/models/<origem>-<destino>/`. **Desvio conhecido:** este requisito pedia verificação de checksum, mas o índice do Argos **não publica hash algum** (verificado: os campos são `package_version`, `from_code`, `to_code`, `links`, `code`). A integridade é estabelecida validando a estrutura do arquivo, recusando caminhos que escapem do destino, e gravando o digest observado na instalação — o que detecta corrupção posterior em disco, mas **não** protege contra adulteração na origem.
...
- **RF-50** — Todo texto dinâmico exibido em widget do Adwaita que interpreta **markup Pango** (título de `Adw.Toast`, descrição de `Adw.PreferencesGroup`, subtítulo de `Adw.ActionRow`) DEVE ser escapado. Texto que o Pango não consegue parsear não é exibido de forma degradada: **não é exibido**. Um atalho como `<Super><Shift>t` vira uma tag desconhecida e o widget renderiza vazio — foi assim que a confirmação de registro do atalho chegou ao usuário como uma caixa sem mensagem.
...
| R13 | Qualidade do modelo offline (OPUS-MT int8) inferior à da API do Google em texto idiomático, e **mistura português europeu e brasileiro de frase para frase** | **Certa** | **Alto** (afeta todo uso por padrão) | A origem do resultado é sinalizada (RF-48); RF-32 permite editar e retraduzir; `google_cloud_v2` continua disponível. **Medido em 2026-08-23:** em 8 frases realistas de interface, 2 saíram europeias ("ficheiro"), 2 brasileiras ("arquivo", "usuário") e 4 neutras. A variante acompanha o comprimento e o contexto: fragmentos curtos tendem a europeu, frases completas a brasileiro. Ver PA-13 |
...
| R16 | Widgets do Adwaita interpretam markup Pango; texto dinâmico não escapado desaparece em silêncio | **Já ocorreu** | Médio | RF-50 obriga escape; teste registra que a forma crua não parseia e a escapada sim |
| R17 | O `.deb` pode omitir dependências que só falham em máquina limpa | **Já ocorreu** (`python3-venv`) | Alto | O workflow de release instala o pacote em contêiner Ubuntu limpo — único teste que prova que o `Depends:` é satisfazível |
...
- `release.yml` — disparado por tag `v*`:
  ...
  3. Constrói o `.deb` (`arch: all`) com **`dpkg-deb` sobre uma árvore montada, não com `debhelper`**. O pacote é Python puro e independente de arquitetura, com poucos arquivos de dados: o conteúdo inteiro cabe em um script legível, `dh-python` acrescentaria dependências de build e comportamento gerado sem ganho nessa escala, e a distribuição é por GitHub Releases e não pelo arquivo Debian. Também permite construir e inspecionar o pacote sem instalar nada, que foi como o layout se verificou.
     O `Depends:` DEVE incluir `python3 (>= 3.10)`, **`python3-venv`**, **`python3-pip`**, `python3-gi`, `python3-gi-cairo`, `gir1.2-gtk-4.0`, `gir1.2-adw-1`, `gir1.2-secret-1`, `python3-pil`, `python3-requests`, `tesseract-ocr` com os pacotes de idioma, e `xdg-desktop-portal`.
     **`python3-venv` e `python3-pip` são obrigatórios e foram esquecidos na v1.0.0.** Sem eles o `--install-engine` não consegue criar o ambiente privado, e o modo de falha é traiçoeiro: `python3 -m venv` monta a árvore de diretórios **antes** de falhar no `ensurepip`, deixando algo que parece instalado e não é. **`ctranslate2` fica fora do `Depends:`** por não existir no APT (RF-42);
...
| **PA-13** | Vale trocar o modelo `en→pt` por um que aceite variante do português? | O pacote Argos atual (`translate-en_pt-1_9`) é rotulado como `pt` genérico e **ignora o token de variante**: `>>pob<<` está no vocabulário, mas não altera a saída nem como token de origem nem como `target_prefix` (ambos testados). O `Helsinki-NLP/opus-mt-tc-big-en-pt` declara `target language(s): pob por` e usa `>>pob<<` para brasileiro, mas exige conversão com `ct2-transformers-converter`, que puxa `torch` e `transformers` — pesado, ainda que só uma vez | Alto para a qualidade percebida; nenhum para a arquitetura, já que o provider não muda |
...
## Adendo à Fase 3 — o que a primeira instalação real ensinou (2026-08-23)

A v1.0.0 passou em 499 testes, na CI e no roteiro de fumaça em contêiner limpo, e ainda assim produziu três defeitos no primeiro uso fora da máquina de desenvolvimento. Os três compartilham uma característica: **falham em silêncio**.

- **RC-29 — Metade de um requisito é pior que nenhuma.** A RF-09 dizia "aciona a captura na instância já em execução via ativação D-Bus". O lado do aplicativo foi implementado (`do_command_line`); o lado da CLI nunca foi ligado. O requisito parecia atendido, e o atalho global simplesmente não fazia nada. A RF-09 foi reescrita nomeando o mecanismo exato, porque a ambiguidade foi o que permitiu implementar metade e considerar pronto.
- **RC-30 — O `Depends:` não é verificável na máquina de quem desenvolve.** `python3-venv` estava presente aqui e ausente lá. Nenhum teste local podia detectar isso; só instalar em contêiner limpo detecta. Isso torna o passo de instalação do workflow de release um requisito, não um luxo.
- **RC-31 — Falhas silenciosas do toolkit exigem teste de integração real.** Markup inválido não degrada: apaga. Um `AdwPreferencesGroup` esvaziado por `get_first_child()` não erra: gira para sempre. Nenhum dos dois é detectável por teste de lógica pura, e ambos chegaram ao usuário. A cobertura de UI passou a construir widgets de verdade.
- **RC-32 — A CI não valida o produto, e isso não é conserto.** Ela roda sem Wayland, sem GNOME Shell, sem portal e sem bandeja. Verde significa "a lógica está correta", nunca "funciona". O `docs/manual-test-plan.md` é o portão real, e o fato de os três defeitos terem sido encontrados por uso humano confirma isso em vez de contradizer.
